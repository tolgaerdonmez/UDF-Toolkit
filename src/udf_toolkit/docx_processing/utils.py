...
def get_bullet_attrs(paragraph):
    ...
    if numPr is not None:
        ...
        if ilvl is not None and numId is not None:
            ...
            return (
                f'Bulleted="true" ListId="{numId.get(qn("w:val"))}" '
                f'ListLevel="{int(ilvl.get(qn("w:val"))) + 1}" BulletType="{bullet_type}"'
            )
    return ''
...
def get_font_properties(run):
    font_family = (
        run.findtext('.//w:rFonts[@w:ascii]', namespaces={'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'})
        or "Times New Roman"
    )
...
def get_line_spacing(paragraph):
    ...
    if spacing is not None:
        ...
        if line and lineRule:
            ...
            elif lineRule in {'exact', 'atLeast'}:
