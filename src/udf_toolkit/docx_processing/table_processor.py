...
def process_table(table, document, current_offset):
    ...
    column_spans = ",".join([str(int((width / total_width) * 300)) for width in column_widths])

    tblBorders = table.find('.//w:tblBorders', namespaces={'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'})
    border_type = "borderCell"
    if tblBorders is not None:
        border_elements = ['top', 'left', 'bottom', 'right', 'insideH', 'insideV']
        all_borders_none = all(
            tblBorders.find(f'.//w:{border}', namespaces={'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'}) is None
            or tblBorders.find(f'.//w:{border}', namespaces={'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'}).get(qn('w:val')) in ['none', 'nil', '0']
    ...
    table_element = (
        f'<table tableName="Sabit" columnCount="{column_count}" columnSpans="{column_spans}" border="{border_type}">'
        f'{"".join(rows)}</table>'
    )
